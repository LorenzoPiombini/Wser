...
static void fe_add(uint64_t a[4],uint64_t b[4],uint64_t result[4]);
static void fe_sub(uint64_t a[4],uint64_t b[4],uint64_t result[4]);
static void fe_mul(uint64_t a[4],uint64_t b[4],uint64_t result[8]);
static uint64_t add64_carry(uint64_t a, uint64_t b, uint64_t *carry);
...
int create_server_hello(struct Client_hello *ch,struct Server_hello *sh)
{
	...
	uint16_t n_group = 0;
	memcpy(&n_group,&ch->ext.saved_extension[bread],sizeof(uint16_t));
	bread += sizeof(uint16_t);
...
int parse_client_hello(struct Client_hello *ch,const uint8_t *buffer)
{
	...
	while(bread < extension_size){
		...
		switch(extension_type){
			...
			{
				/*TODO undrstand if this is necessery or skippable in tls 1.3*/
				uint16_t len = 0;
				memcpy(&len,p,sizeof(uint16_t));
				p += sizeof(uint16_t);
				bread += sizeof(uint16_t);
...
/*EC functions*/
/* 
 * after all this functions you need to make sure that is mod P 
 * where p = 2^255 - 19
 * */
...
static void fe_sub(uint64_t a[4],uint64_t b[4],uint64_t result[4])
{
	...
	uint64_t borrow = 0;
	for(i = 0; i < 4; i++){
		uint64_t borrow_in = borrow;
		result[i] = a[i] - b[i];
		borrow = a[i] < b[i];
		uint64_t t = result[i] - borrow_in;
		uint64_t borrow2 = result[i] < borrow_in;
		borrow = borrow | borrow2;
		result[i] = t;
	}
}
static void fe_sqr(uint64_t a[4],uint64_t result[8])
{
	fe_mul(a,a,result);
}
...
static void fe_mul(uint64_t a[4],uint64_t b[4],uint64_t result[8])
{
	...
	for(j = 0; j < 4; i++){
		for(i = 0; i < 4; i++){
			...
			uint64_t c = 0;
			result[i+j] = add64_carry(lo,hi,&c);
			result[i+j+1] = add64_carry(lo,hi,&c);

			int k = j+i+2;
			while(c && k < 8){
				result[k] = add64_carry(result[k],0,&c);
				k++;
			}
		}
	}
...
static void mul64(uint64_t a,uint64_t b,uint64_t *lo, uint64_t *hi)
{
	...
	uint64_t carry = 0;
	c[0] = add64_carry(c[0],p0,&carry);
	c[1] = add64_carry(c[1],0,&carry);

	uint64_t low = p1 << 32;
	uint64_t high = p1 >> 32;

	carry = 0;
	c[0] = add64_carry(c[0],low,&carry);
	c[1] = add64_carry(c[1],high,&carry);
...
static void fe_inv(uint64_t a[4], uint64_t b[4],uint64_t result[8])
{
